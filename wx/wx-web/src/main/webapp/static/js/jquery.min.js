...
$.extend({
    showLoading: function (msg, dur) {
        if ($('#showloading').length == 0) {
            $('body').prepend('<div id="showloading" class="loading"></div>');
        }
        $('#showloading').text(msg);
        if (!dur) {
            dur = 500;
        }
        $('#showloading').fadeIn(dur);
    },
    hideLoading: function (dur) {
        if ($('#showloading')) {
            if (!dur) dur = 1000;
            $('#showloading').fadeOut(1000);
        }
    },
    ...
    showModel: function (loc) {
        if (loc == '' || loc == undefined || loc == null) {
            loc = '#popModel';
        }
        $(loc).modal('show');
    },
    hideModel: function (loc) {
        if (loc == '' || loc == undefined || loc == null) {
            loc = '#popModel';
        }
        $(loc).modal('hide');
    },
    showConfirm: function (loc, btnYes, callback) {
        if (loc == '' || loc == undefined || loc == null) {
            loc = 'popConfirm';
        }
        $('#' + loc).modal('show');
        if (btnYes == '' || btnYes == undefined || btnYes == null) {
            btnYes = 'btnYes';
        }
    ...
    clearForm: function (parent) {
        ...
        try{
            ...
            textarea.each(function(index,loc){
                $(loc).val(null);
            });
            var select= $(parent).find('select');
            select.each(function(index,loc){
                $(loc).val(null);
            });
        }catch(e){
            console.log(e)
